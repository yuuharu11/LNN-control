...
# Add my function
def filter_obs(obs_dict, obs_keys):
    """
    Filter observation dictionary to only include specified keys.
    
    This is useful for ensuring that only the observation keys expected by a policy
    are passed to it, filtering out any additional observations that may have been
    generated by the environment (e.g., additional sensor data, language embeddings, etc.)
    
    Args:
        obs_dict (dict): dictionary mapping observation keys to np.array or torch.Tensor
        obs_keys (list): list of observation keys to keep. Only observations with keys
            in this list will be retained in the output dictionary.
    
    Returns:
        dict: filtered observation dictionary containing only the specified keys
        
    Example:
        >>> obs = {'robot0_eef_pos': array([...]), 'robot0_joint_pos': array([...]), 'lang_emb': array([...])}
        >>> filtered_obs = filter_obs(obs, ['robot0_eef_pos'])
        >>> print(filtered_obs)
        {'robot0_eef_pos': array([...])}
    """
    return {k: obs_dict[k] for k in obs_keys if k in obs_dict}
