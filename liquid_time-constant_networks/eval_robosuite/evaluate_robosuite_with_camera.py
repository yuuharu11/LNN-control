...
and optionally saves rollout video (via EGL offscreen rendering).
"""
import argparse
import time
import os
...
def parse_args():
    ...
    parser.add_argument("--video_file", type=str, default="rollout.mp4", help="Video output path")
    return parser.parse_args()

def main():
    args = parse_args()

    # GPU レンダリング用に EGL を指定
    ...
    env = suite.make(
        env_name="Lift",
        robots="Panda",
        has_renderer=False,  # 通常のウィンドウレンダリングは不要
        has_offscreen_renderer=args.save_video,  # 動画保存時のみオフスクリーンレンダリング
        use_camera_obs=args.save_video,
        camera_names=["frontview"] if args.save_video else None,
        camera_heights=480,
        camera_widths=640,
        reward_shaping=True,
        use_object_obs=True,
    ...
    for rollout_idx in range(args.num_rollouts):
        ...
        for step in range(args.max_steps):
            ...
            max_memory.append(torch.cuda.max_memory_allocated(args.device) / (1024 ** 2))  # MB
        ...
        # 1本目のロールアウトで動画保存
        if args.save_video and rollout_idx == 0:
            imageio.mimsave(args.video_file, frames, fps=30)
            print(f"✅ Video saved to {args.video_file}")
