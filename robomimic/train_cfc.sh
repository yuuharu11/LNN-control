...
echo "🚀 LNN CfC Training with Multiple Seeds"
...
for SEED in "${SEEDS[@]}"; do
  for DATA_PATH in "${DATASETS[@]}"; do
    ...
    WANDB_NAME="lnn_cfc_seed${SEED}_${DATASET_NAME}"
    EXP_NAME="lift/lnn_cfc/seed${SEED}_${DATASET_NAME}"
    ...
    python /work/robomimic/robomimic/scripts/train.py \
      --name "$EXP_NAME" \
      --dataset "$DATA_PATH" \
      --config /work/robomimic/robomimic/exps/my_params/lnn_cfc.json \
      --seed "$SEED" \
