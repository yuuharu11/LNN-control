...
class LTCQuantizedCell(SequenceModule):
    def __init__(
        self,
        wiring,
        in_features=None,
        ...
        """
        super(LTCQuantizedCell, self).__init__()
    ...
    def _ode_solver(self, inputs, state, elapsed_time):
        ...
        if self.quantize_level is not None:
            cm_t = self.ptq_weight_symmetric(cm_t, n_bits= self.quantize_level)
            vleak = self.ptq_weight_symmetric(vleak, n_bits= self.quantize_level)
            gleak = self.ptq_weight_symmetric(gleak, n_bits= self.quantize_level)
            
        for t in range(self._ode_unfolds):
            w_activation = w_param * self._sigmoid(
                v_pre, self._params["mu"], self._params["sigma"]
            )
            v_pre = self.ptq_weight_symmetric(v_pre, n_bits= self.quantize_level)
