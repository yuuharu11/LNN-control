...
DATASETS=(
    "/work/robomimic/datasets/transport/ph/low_dim_v15.hdf5"
    "/work/robomimic/datasets/transport/mh/low_dim_v15.hdf5"
)
# ✅ UNITS パラメータを追加
UNITS=(128 256 512)
...
for SEED in "${SEEDS[@]}"; do
  for DATA_PATH in "${DATASETS[@]}"; do
    ...
    for UNIT in "${UNITS[@]}"; do
      ...
      EXP_NAME="transport/ncp/${DATASET_NAME}/unit${UNIT}/seed${SEED}"
      ...
      if python /work/robomimic/robomimic/scripts/train.py \
        --name "$EXP_NAME" \
        --dataset "$DATA_PATH" \
        --config /work/robomimic/robomimic/exps/my_params/transport/ncp.json \
        --num_epochs 200 \
        --seed "$SEED" \
        --units "$UNIT" \
