...
class LTCCell(SequenceModule):
    ...
    def _ode_solver(self, inputs, state, elapsed_time):
        ...
        # params
        sensory_w_param = self._params["sensory_w_mask"].to(device)
        w_param = self._params["w_mask"].to(device)
        sensory_w_rev = self._params["sensory_w_rev"].to(device)
        w_rev = self._params["w_rev"].to(device)
